...
area = 1000 #threshold for how much change can be noticed in moving object

while True: #infinite loop
    _,img = cam.read() #reading frame from the camera
    text = "Normal" #no moving object detection
    #pre-processing
    img = imutils.resize(img, width=800) #resize the frame to 500 w
    grayImg = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) #convert color to gray scale image
    gaussianImg = cv2.GaussianBlur(grayImg, (22, 22), 0) #smoothening
    ...
    threshImg = cv2.threshold(imgDiff, 25, 250, cv2.THRESH_BINARY)[1] #detected region will be converted into binary
    ...
    cv2.putText(img, text, (10, 20),
            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 250), 4)
